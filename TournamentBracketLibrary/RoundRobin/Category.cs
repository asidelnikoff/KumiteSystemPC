...
namespace RoundRobin
{
    public class Category : TournamentsBracketsBase.ICategory
    {
        ...
        public void GenerateBrackets()
        {
            ...
            {
                _round.Matches.Add(new Match(new Competitor(pool_1[i]), new Competitor(pool_2[i]), i));
            }
            ...
            for (int i = 1; i < count - 1; i++)
            {
                ...
                for (int j = 0; j < pool_1.Count; j++)
                    round.Matches.Add(new Match(new Competitor(pool_1[j]), new Competitor(pool_2[j]), j));
            }
        ...
        public void UpdateAllRounds()
        {
            ...
            {
                HashSet<Competitor> currentRoundWinners = new HashSet<Competitor>();
                HashSet<Competitor> currentRoundTies = new HashSet<Competitor>();
                foreach (var m in Rounds[round - 1].Matches)
                {
                    if (m.Winner != null && !m.Winner.IsBye)
                        currentRoundWinners.Add(m.Winner as Competitor);
                    else if (m.Winner != null && m.Winner.IsBye)
                    {
                        currentRoundTies.Add(m.AKA as Competitor);
                        currentRoundTies.Add(m.AO as Competitor);
                    }
                }
                ...
                for (int i = round; i < Rounds.Count; i++)
                {
                    foreach (var m in Rounds[i].Matches)
                    {
                        ...
                        if (currentRoundTies.Contains(m.AKA))
                            (m.AKA as Competitor).TotalScore++;
                        if (currentRoundTies.Contains(m.AO))
                            (m.AO as Competitor).TotalScore++;
                    }
                }
            }
        }
        ...
        {
            UpdateRound(round, curMatch);
        }

        private void UpdateRound(int round, int match)
        {
            var winner = Rounds[round].Matches[match].Winner as Competitor;
            var mAka = Rounds[round].Matches[match].AKA as Competitor;
            var mAo = Rounds[round].Matches[match].AO as Competitor;
            if (round + 1 >= Rounds.Count)
                return;

            var competitor = winner;

            for (int i = round + 1; i < Rounds.Count; i++)
            {
                foreach (var m in Rounds[i].Matches)
                {
                    if (winner.Equals(m.AKA as Competitor))
                    {
                        Console.WriteLine("Adding score AKA");
                        (m.AKA as Competitor).TotalScore = competitor.TotalScore + 3;
                        competitor = (m.AKA as Competitor);
                    }
                    else if (winner.Equals(m.AO as Competitor))
                    {
                        Console.WriteLine("Adding score AO");
                        (m.AO as Competitor).TotalScore = competitor.TotalScore + 3;
                        competitor = (m.AO as Competitor);
                    }
                    else if (winner.IsBye)
                    {
                        if (mAka.Equals(m.AKA))
                        {
                            (m.AKA as Competitor).TotalScore = mAka.TotalScore + 1;
                            mAka = (m.AKA as Competitor);
                        }
                        else if (mAka.Equals(m.AO))
                        {
                            (m.AO as Competitor).TotalScore = mAka.TotalScore + 1;
                            mAka = (m.AO as Competitor);
                        }

                        if (mAo.Equals(m.AKA))
                        {
                            (m.AKA as Competitor).TotalScore = mAo.TotalScore + 1;
                            mAo = (m.AKA as Competitor);
                        }
                        else if (mAo.Equals(m.AO))
                        {
                            (m.AO as Competitor).TotalScore = mAo.TotalScore + 1;
                            mAo = m.AO as Competitor;
                        }
                    }
                }
                RoundUpdated?.Invoke(i, Rounds[i].Matches);
            }
        }

        private void ReverseUpdateRound(int round, int match)
        {
            var winner = Rounds[round].Matches[match].Winner as Competitor;
            var mAka = Rounds[round].Matches[match].AKA as Competitor;
            var mAo = Rounds[round].Matches[match].AO as Competitor;
        ...
        public void FinishMatch(int mId, int rId)
        {
            if (rId < Rounds.Count())
            {
                if (Rounds[rId].Matches[mId].Winner == null)
                    Rounds[rId].Matches[mId].CheckWinner();
                if (Rounds[rId].Matches[mId].Winner != null)
                    Rounds[rId].Matches[mId].isFinished = true;
                UpdateRound(rId, mId);
            }
